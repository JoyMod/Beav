...
declare global {
  ...
  interface Window {
    ipcRenderer: {
      ...
      testAiConnection: (config: { apiKey: string; baseURL: string; model?: string; presetId?: string; protocol?: 'openai' | 'anthropic' | 'gemini'; purpose?: 'chat' | 'image' | 'embedding' }) => Promise<{ success: boolean; protocol: 'openai' | 'anthropic' | 'gemini'; models: Array<{ id: string }>; verifiedModel?: string; message: string }>;
